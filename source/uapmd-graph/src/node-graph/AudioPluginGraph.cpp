...
namespace uapmd_graph {
    ...
    uapmd_status_t AudioPluginGraphImpl::appendNodeSimple(int32_t instanceId, uapmd_plugin_hosting::AudioPluginInstanceAPI* instance, std::function<void()>&& onDelete, std::string nodeId) {
        auto newNode = std::make_shared<AudioPluginNodeImpl>(instanceId, instance, event_buffer_size_in_bytes_, std::move(onDelete), std::move(nodeId));
        RTNodeList::ScopedAccess<farbot::ThreadType::nonRealtime> access(nodes_);
