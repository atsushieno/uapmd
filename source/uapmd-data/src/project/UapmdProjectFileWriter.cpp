...
namespace uapmd {
        ...
        AnchorIdGenerator& idGen)
    {
        ...
        if (!markers.empty()) {
            ...
            for (const auto& marker : markers) {
                ...
                markerObj.addMember("position_offset_seconds", marker.clipPositionOffset);
        ...
        if (!audioWarps.empty()) {
            ...
            for (const auto& warp : audioWarps) {
                ...
                warpObj.addMember("offset_seconds", warp.clipPositionOffset);
        ...
        if (clipType == "midi") {
            ...
            if (clip->nrpnToParameterMapping())
                obj.addMember("nrpn_to_parameter_mapping", true);
        }
    ...
    bool UapmdProjectDataWriter::write(UapmdProjectData* data, std::filesystem::path file) {
        ...
        if (!data->settings().empty()) {
            ...
            for (const auto& [name, value] : data->settings())
                settingsObj.addMember(name, choc::json::parse(value));
