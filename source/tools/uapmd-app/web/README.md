...
1. Fetch a local copy of `emsdk` (stored under `source/tools/uapmd-app/web/.emsdk`) if it has not been downloaded yet.
2. Install/activate the requested Emscripten SDK version inside `source/tools/uapmd-app/web/.emsdk`.
...
Generated artifacts live under `cmake-build-wasm/` (for the full build) and `source/tools/uapmd-app/web/dist/` (for the stub). The private SDK cache lives under `source/tools/uapmd-app/web/.emsdk`. Delete them manually if you need to reclaim space.

## Troubleshooting

- **Build fails before configuration**: check that `python3` is installed; the fetch script uses it to run `emsdk.py`.
- **`emcmake` still missing**: delete `source/tools/uapmd-app/web/.emsdk` and rerun the script to force a clean install.
...
The wasm layout mirrors the original `wasm` branch contents while keeping everything scoped under `source/tools/uapmd-app`, so we can iterate without polluting the repo root.
