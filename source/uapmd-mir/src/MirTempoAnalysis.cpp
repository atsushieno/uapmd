...
constexpr double kWindowSeconds = 12.0;
constexpr double kWindowHopSeconds = 3.0;
constexpr double kMinimumWindowSeconds = 8.0;
...
void selectTailHalfTempo(TempoObservation& observation, double duration,
    ...
    // the end, confidence is low, and libsonare reports a nearly tied half-tempo
    // candidate. The selected BPM remains the exact candidate value.
...
double refineTempoTransition(std::span<const float> onset, int sampleRate,
                             double coarseTime, double previousBpm, double nextBpm) {
    if (onset.empty() || sampleRate <= 0)
        return coarseTime;
    const auto framesPerSecond = static_cast<double>(sampleRate) / kOnsetHopLength;
    const auto first = std::max(1, static_cast<int>(std::llround(
        (coarseTime - 6.0) * framesPerSecond)));
    const auto last = std::min(static_cast<int>(onset.size()) - 1,
    ...
    for (auto frame = first; frame <= last; ++frame)
        differences.push_back(
            localPeriodicity(onset, frame, radius, nextLag)
            - localPeriodicity(onset, frame, radius, previousLag));
...
std::vector<std::pair<double, double>> detectTempoMap(
    ...
    for (double start = 0.0; start + kMinimumWindowSeconds <= duration;
         start += kWindowHopSeconds) {
        const auto end = std::min(duration, start + kWindowSeconds);
        ...
        if (error != SONARE_OK || !std::isfinite(analysis.bpm) || analysis.bpm <= 0.0f) {
            sonare_free_bpm_analysis_result(&analysis);
            continue;
        }

        TempoObservation observation{
            start, end, 0.5 * (start + end), analysis.bpm, analysis.bpm, false, {}};
        observation.candidates.reserve(analysis.candidate_count);
        for (size_t index = 0; index < analysis.candidate_count; ++index)
            observation.candidates.emplace_back(analysis.candidates[index].bpm,
        ...
        selectTailHalfTempo(observation, duration, analysis.confidence);
    ...
    if (sonare_onset_strength(samples.data(), samples.size(), sampleRate,
                              2048, kOnsetHopLength, 128,
                              &onsetData, &onsetLength) == SONARE_OK) {
        const std::span<const float> onset{onsetData, onsetLength};
        for (size_t index = 1; index < result.size(); ++index) {
            if (tailHalfSelections[index])
                continue;
            result[index].first = refineTempoTransition(
                onset, sampleRate, result[index].first,
