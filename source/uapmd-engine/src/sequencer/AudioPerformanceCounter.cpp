...
namespace uapmd {
    void AudioPerformanceCounterImpl::setEnabled(bool enabled) {
        enabled_.store(enabled, std::memory_order_relaxed);
    }
    bool AudioPerformanceCounterImpl::enabled() const {
        return enabled_.load(std::memory_order_relaxed);
    }
    uint64_t AudioPerformanceCounterImpl::nextBlockNumber() {
    ...
    bool AudioPerformanceCounterImpl::tryDequeue(AudioProcessingTiming& timing) {
        return timings_.try_dequeue(timing);
    }
