...
- There will be a main audio input bus and a main audio output bus on each plugin instance. They are filled per the rule of its plugin format (mentioned below).
  - They are optional. (VST3 does not support MIDI-only plugins.)
  - On AudioUnit, the default "main" bus would be implicitly assumed to be (1) the only I/O bus, or (2) the first entry of the buses.
- `AudioBusDefinition` contains `role()` which returns `AudioBusRole` enum (`Main` or `Aux`), which is determined as per plugin format:
  ...
  - AudioUnit does not have the concept of "main" bus. What is DAW users experience without default main connection? Do their DAW users have to choose the main bus every time? So far we pick the first bus (if there is at least one) as the main.
- Using `AudioBusConfiguration` members, user determines which buses are enabled and which channel layout is used for each.
...
- `AudioPluginInstance` exposes `PluginAudioBuses` as `audioBuses()`.
- `PluginAudioBuses` exposes list of `AudioBusConfiguration` as `audioInputBuses()` and `audioOutputBuses()`.
