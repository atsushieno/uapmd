...
The engine also observes hosted plug-in state-change notifications. It captures
the resulting opaque project state asynchronously, compares it with the last
known state, and records unsolicited host-UI state edits as one before/after
operation. State notifications generated by replay, restoration and preset
loading update the cache without recursively creating history.
...
- Route every future raw parameter or state surface through the facade's
  callback-based history funnel; no caller may mutate project state directly.
- Add generic begin/end gesture integration for any newly added parameter
...
Plug-in insertion on a new track
...
undo/redo tests. Graph cache invalidation is complete without allowing the
...
- JavaScript parameter, bypass, graph and preset calls now reach the facade;
  unsolicited hosted-UI state notifications are captured by the engine's
  plug-in state-change observer. Any future raw parameter surface still needs
