...
void remidy::PluginInstanceAUv2::ParameterSupport::clearParameterList() {
    for (auto* param : parameter_list)
        delete param;
    parameter_list.clear();
    scoped_parameter_list.clear();
    parameter_id_to_index.clear();
    if (au_param_id_list) {
        free(au_param_id_list);
        au_param_id_list = nullptr;
    }
    au_param_id_list_size = 0;
}

void remidy::PluginInstanceAUv2::ParameterSupport::broadcastAllParameterValues() {
...
void remidy::PluginInstanceAUv2::ParameterSupport::rebuildParameterList(bool notifyListeners) {
    auto impl = [&] {
        UInt32 newListSize = 0;
        auto result = AudioUnitGetPropertyInfo(owner->instance, kAudioUnitProperty_ParameterList,
                                               kAudioUnitScope_Global, 0, &newListSize, nil);
        if (result != noErr) {
            owner->logger()->logError("%s: PluginInstanceAU failed to retrieve parameter list. Status: %d",
                                      owner->name.c_str(), result);
            return;
        }
        ...
        auto* newParamIdList = static_cast<AudioUnitParameterID*>(calloc(newListSize, 1));
        if (!newParamIdList) {
            owner->logger()->logError("%s: PluginInstanceAU failed to allocate parameter list buffer", owner->name.c_str());
            return;
        }

        result = AudioUnitGetProperty(owner->instance, kAudioUnitProperty_ParameterList, kAudioUnitScope_Global, 0,
                                      newParamIdList, &newListSize);
        if (result != noErr) {
            owner->logger()->logError("%s: PluginInstanceAU failed to retrieve parameter list. Status: %d",
                                      owner->name.c_str(), result);
            free(newParamIdList);
        ...
        const size_t totalParameters = newListSize / sizeof(AudioUnitParameterID);
        ...
        std::vector<PluginParameter*> newParameters;
        newParameters.reserve(totalParameters);
        std::unordered_map<AudioUnitParameterID, uint32_t> newParamMap;
        newParamMap.reserve(totalParameters);
        ...
        for (size_t i = 0; i < totalParameters; i++) {
            auto id = newParamIdList[i];
            UInt32 size = sizeof(info);
            result = AudioUnitGetProperty(owner->instance, kAudioUnitProperty_ParameterInfo, kAudioUnitScope_Global,
                                          id, &info, &size);
            if (result != noErr) {
                owner->logger()->logError("%s: PluginInstanceAU failed to retrieve parameter %d (at %zu). Status: %d",
                                          owner->name.c_str(), id, i, result);
                continue;
            }
            ...
            result = AudioUnitGetProperty(owner->instance, kAudioUnitProperty_ParameterValueStrings,
                                          kAudioUnitScope_Global, id, &enumArray, &enumSize);
            ...
            auto p = new PluginParameter(static_cast<uint32_t>(newParameters.size()), idString, pName, path,
                                         info.defaultValue, info.minValue, info.maxValue,
                                         ...
                                         enums);
            const auto newIndex = static_cast<uint32_t>(newParameters.size());
            newParameters.emplace_back(p);
            newParamMap[id] = newIndex;
        }
        ...
        clearParameterList();
        au_param_id_list = newParamIdList;
        au_param_id_list_size = newListSize;
        parameter_list = std::move(newParameters);
        parameter_id_to_index = std::move(newParamMap);
    };
...
void remidy::PluginInstanceAUv2::ParameterSupport::refreshAllParameterMetadata() {
...
void remidy::PluginInstanceAUv2::ParameterSupport::installParameterListener() {
    ...
    AudioUnitEvent parameterListEvent = presetEvent;
    parameterListEvent.mArgument.mProperty.mPropertyID = kAudioUnitProperty_ParameterList;
    AUEventListenerAddEventType(parameter_listener, this, &parameterListEvent);
}
...
void remidy::PluginInstanceAUv2::ParameterSupport::handleParameterEvent(const AudioUnitEvent* event, Float32 value) {
    ...
    if (event->mEventType == kAudioUnitEvent_PropertyChange) {
        auto propertyId = event->mArgument.mProperty.mPropertyID;
        if (propertyId == kAudioUnitProperty_PresentPreset || propertyId == kAudioUnitProperty_ParameterList) {
