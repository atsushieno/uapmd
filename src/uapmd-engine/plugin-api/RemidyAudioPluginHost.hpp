...
namespace uapmd {
    ...
    public:
        ...
        void createPluginInstance(uint32_t sampleRate, uint32_t inputChannels, uint32_t outputChannels, bool offlineMode, std::string &format, std::string &pluginId, std::function<void(AudioPluginInstanceAPI* node, int32_t instanceId, std::string error)>&& callback) override;
