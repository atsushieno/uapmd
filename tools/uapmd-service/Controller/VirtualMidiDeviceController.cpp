...
namespace uapmd {
    ...
    namespace {
        constexpr size_t kDefaultAudioBufferFrames = 4096;
        constexpr size_t kDefaultUmpBufferBytes = 1024;
        constexpr int32_t kDefaultSampleRate = 44100;
    }

    VirtualMidiDeviceController::VirtualMidiDeviceController() {
        sequencer_ = std::make_unique<AudioPluginSequencer>(kDefaultAudioBufferFrames, kDefaultUmpBufferBytes, kDefaultSampleRate);
    }
    ...
    std::shared_ptr<UapmdMidiDevice> VirtualMidiDeviceController::createDevice(const std::string& apiName,
        ...
        auto device = std::make_shared<UapmdMidiDevice>(sequencer_.get(),
                                                        instanceId,
                                                        actualTrackIndex,
                                                        apiName,
                                                        deviceName,
