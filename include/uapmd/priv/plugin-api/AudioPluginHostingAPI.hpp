...
namespace uapmd {
    ...
    public:
        ...
        virtual void createPluginInstance(uint32_t sampleRate, uint32_t inputChannels, uint32_t outputChannels, bool offlineMode, std::string &format, std::string &pluginId, std::function<void(std::unique_ptr<AudioPluginInstanceAPI>, std::string)>&& callback) = 0;
